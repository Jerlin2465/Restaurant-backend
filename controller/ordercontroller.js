...
// // ✅ KEY LINE — GET USER ORDERS (Only logged-in user's orders)
// // Order.find({ userId: req.userId }) — filters by the authenticated user's ID
// // req.userId is injected by your auth middleware after verifying the JWT token
// // This ensures User A can NEVER see User B's orders
// const getUserOrders = async (req, res) => {
//   try {
//     const orders = await Order.find({ userId: req.userId }) // <-- THIS LINE IS THE SECURITY FILTER
//       .populate(
//         "products.productId",
//         "productName price image category description",
//       )
...
//     // ✅ OWNERSHIP CHECK — only the user who placed the order can return it
//     if (order.userId.toString() !== userId.toString())
//       return res.status(403).json({ success: false, message: "Unauthorized" });
